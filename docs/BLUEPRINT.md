...
- `issueBond()` — via `@hashgraph/asset-tokenization-sdk`: name, symbol, ISIN, nominal value, coupon rate, coupon frequency, maturity date, partitions.
...
- **This one check decides whether you need a custom token at all.** If the 2-hour cadence holds, one address yields well over 100 USDC a day and you can run every rehearsal on real Circle USDC with zero self-minted tokens anywhere in the build. If it is really 10 a day, mint `USDX` (6 decimals, in `seed.ts`) for rehearsals only and reserve real USDC for the recorded demo. **Prefer the no-custom-token outcome.** A settlement venue where nothing was minted for convenience is a materially stronger claim.
...
- **Associate every demo account** with USDC, HBAR's counterparts, any `USDX`, and the ATS bond. Unassociated accounts fail transfers with a confusing error. Put all of it in `seed.ts` and run it before every demo.
...
- **Faucet rate limit is unverified.** Sources disagree (20 per 2 hours vs 10 per 24 hours). Test it on day 0. If it turns out to be the slow figure, you cannot catch up later — set a daily reminder immediately.
- **HTS association.** Unassociated account = failed transfer with a confusing error. Seed script associates everything — USDC, `USDX`, and the bond.
